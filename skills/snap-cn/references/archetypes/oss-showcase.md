...
| 360–390f | **CTA** | "Star it on GitHub" text `tracking-in`; repo URL and button `micro-scale-fade`; a soft light sits behind the button |
...
| CTA | `tracking-in` (headline), `micro-scale-fade` (URL + button), `dynamic-grid` (base bg) | **`cta-scene`** — reusable CTA wrapper (label + URL chip + button slot); not in catalog, build as lightweight transparent component (anatomy §1) |
...
- **Background: `dynamic-grid` or a slow muted shader throughout.** A static `dynamic-grid` (~0.06) or a slow, muted shader (`shader-neuro-noise`, `shader-mesh-gradient`) at low `speed` — kept low so it never competes with content. CTA beat lifts the button with a soft, motivated light — a structured editorial card, not a glow blob.
