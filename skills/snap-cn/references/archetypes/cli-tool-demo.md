...
| Agent chat | `v0` / `claude-chat` (match the tool's ecosystem), `cursor` (pointer to input field) | — |
...
| `resultType` | yes | `"pipeline"` → `stepper`; `"metric"` → `rolling-number` + label; `"none"` → skip result beat |
...
- **Pick the agent surface to match the tool's ecosystem.** A Claude-powered tool → `claude-chat`. A web-generation tool → `v0`. Mismatching brand surfaces undermines credibility.
