# snap-cn-ui — component style guide

The contract every `snap-cn-ui` primitive follows. Read this before adding a new
...
> (`pnpm test`, `pnpm run registry:build`, `pnpm dev`) and all commits. Do NOT run
...
grep -nE "useState|useEffect|useCurrentFrame|onClick|onChange|addEventListener|Date\.now|Math\.random|requestAnimationFrame" \
  registry/snap-cn-ui/<name>/index.tsx
```
...
## 5. The timeline (core `lib/snap-cn-ui`)
...
`useSnapCnTheme` / `SnapCnUIProvider` / `SnapCnTheme` (theming), `easings` /
...
- JS theme object with stock shadcn token names is the source of truth
  ...
  both modes for `primary`/`ring`. Radii stay tight: 10px cards, 6–8px controls.
- Resolve with `useSnapCnTheme(override, mode)`. Order: per-component `theme`
  prop > `SnapCnUIProvider` > `defaultLightTheme`/`defaultDarkTheme` per `mode`.
...
- A state atom's registry item ships BOTH `index.tsx` → `components/snap-cn/<name>.tsx`
  AND `use-<name>-transition.ts` → `components/snap-cn/use-<name>-transition.ts`.
- Declare deps: `dependencies` for npm packages (`remotion`, `culori`),
  `registryDependencies` for sibling registry items (`["snap-cn-ui", "spinner", …]`).
- Source files import siblings by the **install path** (`@/components/snap-cn/<name>`),
  not the repo path — that ships correctly to users and resolves in-repo via the
  tsconfig alias `@/components/snap-cn/* → registry/snap-cn-ui/*/index.tsx`.
- `config.ts` is preview-only — do NOT list it in registry files.
- The shared core (`snap-cn-ui` registry:lib, sourced from `core/`) is pulled
...
- Tests run via `pnpm test` (vitest, root `vitest.config.mts` — it resolves the
  tsconfig `paths` aliases) and must pass before every commit.
...
- Mirror impure folds (anything calling `useCurrentFrame`) as pure replicas with
  ...
  them in lockstep.
- Use relative imports for component source, `@/lib/snap-cn-ui` for the core
